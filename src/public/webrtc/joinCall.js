const url = "https://oflepconnect.brightcodeapp.com";
let meetingDetails;
let displayName;
let localStream;
...
socket.on("connect", async () => {
  ...
  const isVerified = await verifyMeetingDetails(meetingCreds);
...
socket.on("someone-got-connected", async (data) => {
  const callee = data.joineeId;

  if (callee != userId) {
    console.log("someone got connected", data);
    await startCalling(callee);
  }
});
...
async function verifyMeetingDetails(meetingDetails) {
...
async function startCalling(to) {
  // grab local stream
